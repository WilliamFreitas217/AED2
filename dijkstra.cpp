...
Dijkstra::Dijkstra(Grafo g, int s){
	...
	while(Q.getTamHeap()>0){
        ...
        // Par *pares = new Par[vetor.getTam()+1];
        // int i=1;
        int pred, ant;
		while(v!=NULL){
            cout<<"\tvertice: "<<u.getVertice()<<"\tadjacente: "<<v->getItem().getVertice()<<"\t Peso: "<<g.w(u.getVertice(), v->getItem().getVertice())<<endl;
			relaxa(g, u.getVertice(), v->getItem().getVertice());
            pred = u.getVertice();
            ant = v->getItem().getVertice();
			v= v->getProx();
		}
        cout<<"De "<<pred<< " vai pra "<< predecessores[pred]<<endl;
	}
}
...
void testaGrafo(Grafo &g){
	...
	g.mostra();
    dijkstra = new Dijkstra(g, 11);
    // cout<<"predecessores"<<endl;
    // printar(dijkstra->getPredecessores(), g.getN());
    // cout<<"distancias"<<endl;
    // printarVetor(dijkstra->getDistancias(), g.getN());
}
int main(){
    int n, m;
	cin>>n>>m;
