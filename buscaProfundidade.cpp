...
public:
    ...
    void dfsVisita(Grafo<Vertice> &g, int u);
};
...
void BuscaProfundidade::dfsVisita(Grafo<Vertice> &g, int u){
    tempo++;
    tempoD[u] = tempo;
    cores[u]=cinza;
    // cout<<u<<endl;
    int i = 1;
    Lista<Vertice> adj = g.getAdj()[u];
    No<Vertice> *v = adj.getPrim()->getProx();
    while(i<=adj.getTam()){
        if(cores[v->getItem().getChave()]==branco){
            v->getItem().setVerticePredesc(u);
            dfsVisita(g, v->getItem().getChave());
        }
        v = v->getProx();
        i++;
    }
