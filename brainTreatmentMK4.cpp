...
public:
    VertexDij();
    VertexDij(int chave, int distancia);
...
VertexDij::VertexDij(int chave, int distancia){
    this->chave = chave;
    this->distancia = distancia;
}

int VertexDij::getChave(){
    return chave;
}

int VertexDij::getDistancia(){
    return distancia;
}

void VertexDij::setChave(int chave){
    this->chave = chave;
}

void VertexDij::setDistancia(int distancia){
    this->distancia = distancia;
}
...
void Kruskal::mstKruskal(BlocoNeuronio &blocoDeNeuronio){
    ...
    quickSort(arestas, 1, tamVetorArestas);
    // Heap<Trio> heap;
