...
void printarVetor(Tipo *vetor, int tam){
    for(int i=0; i<tam; i++){
        cout<<"["<<i<<"] :: "<< "\tv: "<< vetor[i].getVertice()<<"\t w:"<< vetor[i].getPesoAresta()<<endl;
    }
}
...
void FilaDePrioridade::decrescerMin(int i, Par par){
    ...
    int iAux = i;
    for(int j=tamHeap; j>0; j++){
        while ((i!=0) && (vetor[pai(i)].getPesoAresta()>vetor[j].getPesoAresta())) {
            troca(vetor[pai(i)], vetor[j]);
            i = pai(i);
        }
        i = iAux;
    }
...
void FilaDePrioridade::insere(Par par){
    tamHeap++;
    vetor[tamHeap-1].setVertice(par.getVertice());
    vetor[tamHeap-1].setPesoAresta(par.getPesoAresta());
    int i = tamHeap-1;
    int iAux = i;
    // cout<<"aqui"<<endl;
    // for(int j=tamHeap-1; j>3; j++){
    //     while ((i!=0) && (vetor[pai(i)].getPesoAresta()>vetor[j].getPesoAresta())) {
    //         troca(vetor[pai(i)], vetor[j]);
    //         i = pai(i);
    //     }
    ...
    while ((i!=0) && (vetor[pai(i)].getPesoAresta()>vetor[i].getPesoAresta())) {
        troca(vetor[pai(i)], vetor[i]);
        i = pai(i);
    }
    printarVetor(vetor, tamHeap-1);
}
